...
async function submitForm() {
    const formData = {
        ...
        pace: document.querySelector('.question:nth-child(6) input[type="range"]')?.value || 1,
        ...
        budget: document.querySelector('.question:nth-child(9) input[type="range"]')?.value || 0
    };
...
document.querySelectorAll('.question button').forEach(button => {
    button.addEventListener('click', function() {
        // 如果不是第4个问题，确保同一问题只选一个按钮
        if (!this.parentElement.classList.contains('question:nth-child(4)')) {
            const selectedButton = this.parentElement.querySelector('button.selected');
